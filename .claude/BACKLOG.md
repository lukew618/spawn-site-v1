...
- [ ] **Skip-to-content link hidden** — `layout/theme.liquid` has a skip-to-content link with `hidden` attribute. Remove `hidden`, style it as visually-hidden by default and visible on focus (WCAG 2.1 AA). Acceptance: keyboard Tab from any page focuses the skip link first, pressing Enter jumps to `#MainContent`.
